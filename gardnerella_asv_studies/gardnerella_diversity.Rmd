...
author: "Hanna Berman & Megan Anderson"
output:
  html_document:
    toc: true
    toc_float: true
...
v4_study_table <- sample_counts %>%
  ...
  kbl(caption = "Table S3. V4 16S Amplicon Studies") %>%
  kable_classic(full_width=TRUE, html_font = "Arial")

v4_study_table
#save_kable(v4_study_table, file = file.path(figureOut, paste(Sys.Date(), "TableS3_metadata.png", sep="_")), zoom=5)
```
...
ASVperSample <- v4DF %>%
  ...
  labs(x="Gardnerella ASVs per sample")
ASVperSample
```
...
#ggsave(filename = file.path(figureOut, paste(Sys.Date(), "Figure7_multistudyASVs.png")))
```
