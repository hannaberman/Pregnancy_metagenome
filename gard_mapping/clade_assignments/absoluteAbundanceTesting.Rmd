...
aaCladesPlot <- DF %>%
  ...
  gather("Clade", "Abundance", c(G_vaginalis, C1, C2, C3, C4, C5, C6)) %>%
...
corTestFig <- function(df, name){
  ...
  plot <- cor_test_results %>%
    filter(Taxon1!=Taxon2,
      ...
      mutate(p.value=case_when(p.value>0.01~"",
                             p.value<=0.01&p.value>0.001~"*",
                             p.value<=0.001&p.value>0.0008~"**",
                             p.value<=0.0008&p.value>0.0001~"***",
                             p.value<=0.0001~"****"),
          Taxon1=factor(Taxon1, levels=c("G_vaginalis", taxaOfInterestAbbr)),
          Taxon2=factor(Taxon2, levels = c(rev(taxaOfInterestAbbr), "G_vaginalis"))) %>%
    ggplot(aes(x=Taxon1, y=Taxon2, fill=estimate, label=p.value)) +
      ...
      facet_wrap(~Cohort) +
      theme(axis.text.x = element_text(angle=90)) +
      labs(x="Taxon 1 ", y="Taxon 2", title = "Abundance Correlations", subtitle = name)
...
PresAbsVsAbundance <- function(df, name){
 df0 <- df %>%
    ...
    mutate(presAbs=case_when(presAbs>0~"present", 
           ...
           Taxon=factor(Taxon, levels=c("G_vaginalis", taxaOfInterestAbbr), labels=c("Gv", "C1", "C2", "C3", "C4", "C5", "C6", "Lc", "Lg", "Lj", "Li", "Av", "Mh", "Pb", "Up"))) 
  wilcoxResults <- df0 %>%
  ...
  plot <- df0 %>%
    ggplot(aes(x=presAbs, y=totalAbundance)) +
      ...
      scale_y_log10() +
      theme(axis.text.x = element_text(angle=90)) +
      labs(x=element_blank(), y="Total Abundance", title="Presence-Absence vs. Abundance", subtitle = name)
  outputList <- list(`Wilcox Results`=wilcoxResults, Figure=plot)
}
