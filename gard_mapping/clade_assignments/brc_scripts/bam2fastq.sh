...
#files without mapq filter
samtools sort \
    -n \
    -o $unf_bam_path/${accession}_FgardRef_n.bam \
    $unf_bam_path/${accession}_FgardRef.bam
samtools fastq \
    -1 $out_dir_unf/${accession}_FgardRef_1.fastq.gz \
    -2 $out_dir_unf/${accession}_FgardRef_2.fastq.gz \
    -s $out_dir_unf/${accession}_FgardRef_S.fastq.gz \
    $unf_bam_path/${accession}_FgardRef_n.bam
