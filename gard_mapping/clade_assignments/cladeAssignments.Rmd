...
ggplot(data = QueriesPerAnnotation, aes(queriesbyannotation)) +
  geom_histogram() +
  theme_classic() +
...
#ggsave(file.path(gdrive_path, "figures", "queriesperannotation.pdf"))
...
# How does filtering affect clade assignments? -- filtering at 99%
```{r}
alns99 <- alns0 %>%
...
``` 
Note - many samples have few reads (<300) that map to G. vaginalis after filtering. Based on barplots of annotations found within each sample, it seems to be apparent that annotations with references that belong to different clades account for all of the alignments to some clades (ie clades not actually present but an artifact of ambiguous alingment.) 
...
# Figure for comparison with 16S: 
...
ggplot(data=compare16S, aes(x=GWcoll, y=propClade, color=Clade, shape=RNAlater)) +
  ...
  labs(title = "Clade assignments by shotgun sequencing", x= "GW collection", y="Proportion of Gardnerella", shape="RNA Later", color="Clade")
