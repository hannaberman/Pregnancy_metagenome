...
Where $N =$ the numner of sites surveyed, $N_1=$ the number of sites where taxon 1 occurrs, and $N_2=$ the number of sites where taxon 2 occurrs. Adding a 0.1% threshold for true presence.
...
DFpa <- DF %>% #presence, absence DF
  mutate_if(is.numeric, 
            ~case_when(.x>0.001~1,
                       .x<0.001~0)) %>%  # threshold by .01% (0.001. value used in Fettweis et al. 2019 and Ansicar et al. 2019)
...
```
Note: no *L. crispatus* in any of the randomly selected subset samples in the UAB cohort. All of these samples have and total Gard. After ading threshold, not all samples in UAB subset have C1.
...
No major changes in Stanford  samples after thresholding. Exclusion between *L. iners* and *L. gasseri* now significant. *L. iners* and C5 relationship gets stronger. C5 and *M. hominis* co-occurrence is no longer significant.

In UAB samples, C1 and *A. vaginae* significantly co-ccurr, and C5 and C6 exclude *P. bivia*.
...
testDists <- mDFG %>%
  ...
  mutate_if(is.numeric, ~case_when(.x<0.001~"Absent",
                                   .x>0.001~"Present")) %>%
...
DFpa %>%
  left_join(sgDF[,c("SampleID", "TotalReads", "upload_pe", "bbmapFiltered_pe")]) %>%
...
DFpa %>%
  ...
  mutate(PresAbs=case_when(PresAbs==0~"Absent",
         ...
         Organism=case_when(Organism=="Gardnerella_vaginalis_C1"~"C1",
                            ...
                            Organism=="Ureaplasma_parvum"~"U_parvum",
                            Organism=="Mycoplasma_hominis"~"M_hominis")) %>%
  ggplot(aes(x=as.character(PresAbs), y=upload_pe)) +
  geom_boxplot() +
  geom_jitter() +
  theme(axis.text.x = element_text(angle=45, hjust=1)) +
  facet_grid(Cohort~Organism) +
  labs(x="Presence vs. Absence", y="Reads")
```
Number of uploaded pairs (what was put through the pipeline) comparing presence vs. absence of each taxon. No consistent trends. 
...
DFpa %>%
  ...
  mutate(PresAbs=case_when(PresAbs==0~"Absent",
         ...
         Organism=case_when(Organism=="Gardnerella_vaginalis_C1"~"C1",
                            ...
                            Organism=="Ureaplasma_parvum"~"U_parvum",
                            Organism=="Mycoplasma_hominis"~"M_hominis")) %>%
  ggplot(aes(x=as.character(PresAbs), y=bbmapFiltered_pe)) +
  geom_boxplot() +
  geom_jitter() +
  theme(axis.text.x = element_text(angle=45, hjust=1)) +
  facet_grid(Cohort~Organism) +
  labs(x="Presence vs. Absence", y="Reads")
```
Number of reads after bbmap filtering. (Not input into pipeline, but a better representation of bacterial reads per sample). May be some increase in presence vs. absence in some cases, but there are taxa with few absent/few present cases. 
...
DFpa %>%
  left_join(sgDF[,c("SampleID", "TotalReads", "upload_pe", "bbmapFiltered_pe")]) %>%
  gather("Organism", "PresAbs", c("Gardnerella_vaginalis", cladesSpecies)) %>%
  gather("Method", "Reads", c("TotalReads", "upload_pe", "bbmapFiltered_pe")) %>%
  mutate(PresAbs=case_when(PresAbs==0~"Absent",
                           PresAbs==1~"Present")) %>%
