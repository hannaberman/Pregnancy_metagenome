...
sgGardDF <- "./gardGenomospeciesRelativeAbundance.tsv" %>%
  read_tsv() %>%
  mutate(SampleID=as.character(SampleID))
...
ggplot(aes(x=GWcoll, y=propGard, shape=Method, size=Method, color=Var)) +
  geom_point() +
  scale_size_manual(values=c(1.5, 3)) +
  scale_color_manual(values=c("salmon", "salmon3", "indianred", "indianred1", "blue4", "blue", "lightblue1", "lightblue2", "lightblue3", "lightblue4",  "indianred2", "darkorchid2", "yellowgreen", 
                     ...
                     labels=c("Genomospecies 1, G. vaginalis", 
                                 ...
                                 "Genomospecies 12", 
                                 "Genomospecies 13",
                                 "G2: Clades 1/2 (V4 amplicon)", "G1: Clades 3/4 (V4 amplicon)", "G3: indeterminant (V4 amplicon)", "G4: Clade 5 (V4 amplicon)", "G5: Clade 6 (V4 amplicon)")) + 
  scale_shape_manual(values = c(16, 1)) +
...
tribble(
        ...
        "Percent V4 variants not represented", "14.3", "17.9") %>%
  formattable
```
