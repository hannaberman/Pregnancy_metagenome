...
mDF %>%
  ...
  left_join(sgDF[,c("SampleID", "finalPctHuman_po", "finalPctHuman_all", "bbmapFiltered_pairs", "bbmapFiltered_sngl")], by="SampleID") %>%
  ggplot(aes(x=bbmapFiltered_pairs, y=nTaxa)) +
    geom_point()
```

## Minimum Reads vs. percent human (shotgun, gard only)
```{r}
sgGardDF %>%
  group_by(SampleID) %>%
  summarise(minReads=min(n)) %>%
  left_join(sgDF[,c("SampleID", "finalPctHuman_po", "finalPctHuman_all", "bbmapFiltered_pairs", "bbmapFiltered_sngl")], by="SampleID") %>%
  ggplot(aes(x=bbmapFiltered_pairs, y=minReads)) +
    geom_point()
```

## Minimum Reads vs. percent human (Callahan et al 2017, amplicon all taxa )
```{r}
PS16S_170 %>%
  psmelt() %>%
  filter(Abundance>0) %>%
  group_by(SampleID) %>%
  summarise(minReads=min(Abundance)) %>%
  left_join(sgDF[,c("SampleID", "finalPctHuman_po", "finalPctHuman_all", "bbmapFiltered_pairs", "bbmapFiltered_sngl")], by="SampleID") %>%
  ggplot(aes(x=bbmapFiltered_pairs, y=minReads)) +
    geom_point()
```
