...
gardRelativeAbundance %>%
  ...
  gather("method","value", Metaphlan, Mapping) %>%
  ggplot(aes(x=SampleID, y=value, group=method, color=method)) +
    geom_line() +
    theme(axis.text.x = element_blank()) +
    scale_y_log10() +
    labs(y="Fraction of Sample", x="Sample")
```


## number of RNAlater samples with multiple clades
```{r}
gardRelativeAbundance %>%
  select(SampleID, Clade) %>%
  left_join(sgDF[,c("SampleID", "Cohort", "RNAlater")], by="SampleID") %>%
  filter(RNAlater=="Yes") %>%
...
sgDF0 %>%
   ggplot(aes(x=nClade)) +
    ...
    scale_x_continuous(breaks = c(-1,0,1,2,3,4,5,6), labels = c("NA", 0, 1, 2, 3, 4, 5, 6)) +
    facet_wrap(~Cohort)+
...
Observe proportion of clades vs other clades. Use clade-positive samples only.
...
Ratio of each clade to other Gard types
```{r}
gardRelativeAbundance1 %>%
...
```
  ...
  Subject average clade proportion vs other gard proportion
```{r}
...
Gard clade : lactos vs gard clade + lactos
...
Ratio of gard clades to Lactos
```{r}
#Ratio of each clade to each lacto in term pre
taxAverages %>%
  spread(Taxon, taxAvg) %>%
  gather("gardClade", "gardAvg", c(`C1`,`C2`,`C3`,`C4`,`C5`,`C6`, `Clade uncharacterized`)) %>%
  gather("lacto", "lactoAvg", c(`L. crispatus`, `L. gasseri`, `L. iners`, `L. jensenii`)) %>%
...
taxAverages %>%
  spread(Taxon, taxAvg) %>%
  gather("gardClade", "gardAvg", c(`C1`,`C2`,`C3`,`C4`,`C5`,`C6`, `Clade uncharacterized`)) %>%
  mutate(lactoSum=`L. crispatus`+`L. gasseri`+`L. jensenii`,
...
top20abundance <- gardRelativeAbundance1 %>%
  ...
  spread(Clade, propCladeCommunity) %>%
  gather("Taxon", "Abundance", c(`C1`, `C2`, `C3`, `C4`, `C5`, `C6`, Uncharacterized,  top19)) %>%
  mutate(Taxon=case_when(Taxon=="Uncharacterized"~"G. vaginalis uncharacterized",
                         Taxon!="Uncharacterized"~Taxon))
...
New dataframe not saved on 5/12/2020
```{r}
