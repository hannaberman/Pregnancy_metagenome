...
taxColorsOther <-   list(scale_color_manual(values=c("#808080", "#CC79A7", "#D55E00", "#56B4E9", "#0072B2", "#E69F00", "#009E73",
                                                 "darkgreen", "khaki1", "darkred", "darkblue",
                                                 "mediumpurple4", "#C42104",  "darkseagreen4", "black"),
...
# Set up taxa data frames
  Organize and set up tables from MetaPhlAn4 and *Gardnerella* mapping method.
## MetaPhlAn4
...
sgDF %>%
  mutate(Cohort=factor(Cohort, levels=c("Stanford Enriched", "UAB Enriched", "MOMS-PI", "MOMS-PI Enriched")),
         TermPre=factor(TermPre, levels=c("T", "P"))) %>%
...
```
  ...
  Samples per subject
```{r, message=FALSE, range=FALSE}
sgDF %>%
  mutate(Cohort=factor(Cohort, levels=c("Stanford Enriched", "UAB Enriched", "MOMS-PI", "MOMS-PI Enriched")),
         TermPre=factor(TermPre, levels=c("T", "P"))) %>%
  group_by(Cohort, TermPre) %>%
  count(SubjectID) %>%
  summarize(meanSwabs=round(mean(n),1), sdSwabs=round(sd(n),1), rangeSwabs=paste(range(n), collapse = ";")) %>%
  formattable
```
...
```
  ...
  Table S3: MOMS-PI- Enriched demographic information
```{r, message=FALSE, warning=FALSE}
MEgaadDF <- sgDF %>%
  filter(Cohort=="MOMS-PI Enriched"|Cohort=="Stanford Enriched") %>%
  select(SubjectID, Cohort, TermPre, GWdel) %>%
  unique() %>%
  add_count(Cohort, TermPre) %>%
  group_by(Cohort, TermPre, n) %>%
  ...
  as.data.frame %>%
  rownames_to_column(var="x") %>%
  select(`x`,  `MOMS-PI Enriched\nTerm (n=27)`, `MOMS-PI Enriched\nPreterm (n=15)`)
...
MEdemdf <- sgDF %>%
  ...
  arrange(x) %>% # order rows
  mutate(x=str_extract(x, ".*(?=_)")) %>% # remove category label now that rows are arranged
  select(`x`,  `MOMS-PI Enriched\nTerm (n=27)`, `MOMS-PI Enriched\nPreterm (n=15)`) # order columns

MEdemTable <- MEgaadDF %>%
  rbind(MEdemdf) %>%
  dplyr::rename("  "=x) %>%
  dplyr::rename_at(2:3, ~str_extract(.x, "(?<=\\n).*")) %>%
  kbl(caption = "Table S3. MOMS-PI Enriched Demographic Information") %>%
  kable_classic(full_width=TRUE, html_font = "Arial") %>% 
  add_header_above(c(" ", "MOMS-PI Enriched" = 2)) %>%
  pack_rows("Age", 2, 6) %>%
  pack_rows("Race", 7, 10) %>%
  pack_rows("Ethnicity", 11, 13) %>%
  pack_rows("Education", 14, 19) %>%
...
# Thresholds for presence-absence
  ...
  MetaPhlAn will report all taxa that have 10% of marker genes non-zero [based on this Google group response from Nicola Segata](https://groups.google.com/g/metaphlan-users/c/0jLFs8v9nI4?pli=1). Average number of marker genes per bacterial species is 184 ± 45. (Truong et al., 2015, *Nature Mathods* "MetaPhlAn4 for enhanced metagenomic taxonomic profiling"). About 1 million markers from >7,500 species. Minimum number of reads to be detected would be 18.4. 
```{r, message=FALSE, warning=FALSE}
sgDF1 %>%
...
```
  ...
  Model: $y_{i}=\beta_{0} + x_{i}\beta_{1}+e{i}$
```{r, message=FALSE, warning=FALSE}
