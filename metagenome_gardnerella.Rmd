...
## Define file paths
```{r, message=FALSE}
figureOut <- "../FIGURES/V3_manuscript_figures/cohorts_and_analyses"
...
sgDF1 %>%
  ...
  labs(x=NULL, y="Paired Reads")
#ggsave(filename = file.path(figureOut, paste(Sys.Date(), "pairedReadsAtSteps.png", sep="_")))
```
...
readsTable <- sgDF1 %>%
  ...
  transmute(Cohort=Cohort,
            ...
            `Bacterial Reads`=paste0(bbmapFiltered_pairs_mean, " (", bbmapFiltered_pairs_sd, ")"),
            `Percent Human Reads`=paste0(pctHuman_mean, " (", pctHuman_sd, ")")) %>%
  kbl() %>%
  kable_classic(full_width=TRUE, html_font = "Arial")
readsTable
#save_kable(readsTable, file = file.path(figureOut, paste(Sys.Date(), "readsTable.png", sep="_")))
```
...
samplingSched <- sgDF1 %>%
  ...
  mutate(GWcoll=GDcoll/7,
         GWcoll=floor(GWcoll),
         Cohort=factor(Cohort, levels=c("Stanford Enriched", "UAB Enriched", "MOMS-PI"), labels=c("Stanford Enr.", "UAB Enriched", "MOMS-PI"))) %>%
  split(.$Cohort) %>%
  map2(names(.), ~ggplot(.x) +
...
demTable
#save_kable(demTable, file = file.path(figureOut, paste(Sys.Date(), "demTable.png", sep="_")))
```
...
q <- hmp2mDF %>%
  ...
  theme(legend.position = "bottom") +
  binaryColors +
  labs(x="Proportion Gardnerella", y="Bacterial Reads", color="Uncharacterized Gardnerella", shape="Uncharacterized Gardnerella")
...
subsetNs <- sgDF %>%
  ...
  count(Cohort) %>%
  dplyr::rename(N=n) %>%
  kbl(caption = "Table S3. One Sample Per Subject Subset") %>%
  kable_classic(full_width=TRUE, html_font = "Arial")
subsetNs
#save_kable(subsetNs, file = file.path(figureOut, paste(Sys.Date(), "subsetNsTable.png", sep="_")))
```
...
```
  More clades per sample in UAB and enriched MOMS-PI cohorts than Stanford and MOMS-PI cohorts. 
...
prevTable %>%
  ...
  ggplot(aes(x=var, fill=var, y=m)) +
  geom_bar(stat="identity") +
  geom_text(aes(y=0.72, x=var, label=prevalence), color="black", size=5) +
  cladeColorsTotalUnch +
  facet_grid(Cohort~.) +
  scale_y_continuous(breaks=c(0.0,0.2,0.4,0.6,0.75), limits=c(0,0.8), labels=c("0.0","0.2","0.4","0.6","prevalence")) +
  theme(legend.position = "none",
        axis.text.x = element_text(size = 16),
        axis.text.y = element_text(size=16),
        axis.title.y = element_text(size=23),
...
```
  Mean relative abundance and prevalence with binomial test
```{r}
prevTabBinom <- gardCladeAbU %>%
  with_groups(c(Cohort, var), summarize, p = sum(relativeAbundance > 0), n = n(), m=mean(relativeAbundance)) %>% # p = # times clade is present, n = number samples, m=mean relative abundance
  mutate(
    test = map2(p, n, binom.test),
    test = map(test, tidy)) %>%
  unnest(test) %>%
  gather("measure", "value", c(m, estimate)) %>%
    mutate(var=factor(var, levels=c("Gardnerella_vaginalis", "Uncharacterized_Gardnerella", rev(clades)), labels=c("Total", "Unch.", rev(clades))),
           measure=factor(measure, levels=c("m", "estimate"), labels=c("Mean Relative Abundance", "Prevalence"))) 

ggplot() +
  geom_pointrange(data=subset(prevTabBinom, measure=="Prevalence"), aes(y=var, x=value, xmin=conf.low, xmax=conf.high, group=Cohort, color=Cohort), position=position_dodge2(width=1, reverse=TRUE)) +
  geom_bar(data=subset(prevTabBinom, measure=="Mean Relative Abundance"), aes(y=var, x=value, group=Cohort, fill=Cohort), position=position_dodge2(width = 1, reverse=TRUE), stat="identity") +
  ...
  theme(axis.text.x = element_text(size = 14),
        ...
        strip.text = element_text(size=13)) +
  labs(x=NULL, y=NULL)
#ggsave(file.path(figureOut, paste(Sys.Date(), "meanRelAbundanceAndPrevBimom.png", sep="_")))
```

  All sample relative abundance and prevalence with binomial test
```{r}
prevTabBinom2 <- gardCladeAbU %>%
  with_groups(c(Cohort, var), summarize, p = sum(relativeAbundance > 0), n = n(), m=mean(relativeAbundance)) %>% # p = # times clade is present, n = number samples, m=mean relative abundance
  mutate(
    test = map2(p, n, binom.test),
    test = map(test, tidy)) %>%
  unnest(test) %>%
  select(Cohort, var, estimate, conf.low, conf.high) %>%
  full_join(gardCladeAbU, by=c("Cohort", "var")) %>%
  gather("measure", "value", c(relativeAbundance, estimate)) %>%
    mutate(var=factor(var, levels=c("Gardnerella_vaginalis", "Uncharacterized_Gardnerella", rev(clades)), labels=c("Total", "Unch.", rev(clades))),
           measure=factor(measure, levels=c("relativeAbundance", "estimate"), labels=c("Relative Abundance", "Prevalence")),
           Cohort=factor(Cohort, levels=cohorts)) %>%
  select(-SampleID) %>%
  unique

ggplot() +
  geom_pointrange(data=subset(prevTabBinom2, measure=="Prevalence"), aes(y=var, x=value, xmin=conf.low, xmax=conf.high, color=Cohort), position=position_dodge2(width=1, reverse=TRUE)) +
  geom_boxplot(data=subset(prevTabBinom2, measure=="Relative Abundance"), aes(y=var, x=value, color=Cohort, fill=Cohort), alpha=0.5, position=position_dodge2(reverse=TRUE)) +
  scale_x_continuous(breaks=seq(0,1,0.25), labels = c("0", "0.25", "0.5", "0.75", "1")) +
  facet_wrap(~measure) +
  quadColors +
  theme(axis.text.x = element_text(size = 14),
        ...
        strip.text = element_text(size=13)) +
  labs(x=NULL, y=NULL)
#ggsave(file.path(figureOut, paste(Sys.Date(), "claderelAbundanceAndPrevBimom.png", sep="_")))
```

  Sample relative abundance and prevalence
```{r, fig.width=2.5, fig.height=4}
gardCladeAbU %>%
  mutate(var=factor(var, levels=c("Gardnerella_vaginalis", "Uncharacterized_Gardnerella", rev(clades)), labels=c("Total", "Unch.", rev(clades)))) %>%
  ggplot(aes(x=relativeAbundance, y=var, fill=Cohort, color=Cohort)) +
  geom_boxplot(alpha=0.5, position=position_dodge2(reverse = TRUE)) +
  geom_text(data=prevTable, aes(x=1.25, label=prevalence), position=position_dodge2(width=0.75, reverse = TRUE), size=3.5, show.legend = FALSE) +
  scale_x_continuous(breaks = c(0,0.25,0.5,0.75,1,1.25), labels=c("0", "0.25", "0.5", "0.25", "1", "Prev."), limits=c(0,1.3)) +
  theme_bw() +
  quadColors +
  theme(axis.text.x = element_text(size = 14),
        axis.text.y = element_text(size=14),
...
#ggsave(file.path(figureOut, paste(Sys.Date(), "cladeRelativeAbundanceAndPrevByClade.png", sep = "_")))
```
...
gardGenomoAbU %>%
  ...
  mutate(var=factor(var, levels=c(genomosCladeOrder, "Uncharacterized_Gardnerella", "Gardnerella_vaginalis"), labels = c(genomoNamesCladeOrder, "Unch.", "Total")),
         Cohort=factor(Cohort, levels=cohorts, labels = cohortAbbrevs)) %>%
  ggplot(aes(x=var, y=relativeAbundance, color=var, fill=var)) +
  geom_quasirandom(alpha=0.5) +
  geom_boxplot(color="black", alpha=0) +
...
gardGenomoAbU %>%
  ...
  mutate(Genomospecies=factor(Genomospecies, levels = c(genomosCladeOrder, "Uncharacterized_Gardnerella", "Gardnerella_vaginalis"))) %>%
...
#ggsave(file.path(figureOut, paste(Sys.Date(), "meanRelAbundanceVsPrevalenceGenomos.png", sep="_")))
```
...
mDF %>%
  ...
  split(.$SampleID)
```
  Many have Gardnerella, all have Prevotella timonensis or amnii
...
jaccardsSubset$`Stanford Enriched`[lower.tri(jaccardsSubset$`Stanford Enriched`, diag = TRUE)] <- NA
jaccardsSubset$`UAB Enriched`[lower.tri(jaccardsSubset$`UAB Enriched`, diag = TRUE)] <- NA
jaccardsSubset$`MOMS-PI Enriched`[lower.tri(jaccardsSubset$`MOMS-PI Enriched`, diag = TRUE)] <- NA
jaccardsSubset$`MOMS-PI`[lower.tri(jaccardsSubset$`MOMS-PI`, diag = TRUE)] <- NA
...
## Subject average  abundance by PTB status
  Wilcoxon rank-sum: Relative abundance
```{r}
relativeWilcoxResults <- momsptbDF %>%
  filter(Cohort=="MOMS-PI") %>%
  with_groups(c(SubjectID, Cohort, TermPre, var), summarize, meanRA=mean(relativeAbundance)) %>%
  group_by(var) %>%
  wilcox_test(meanRA~TermPre, alternative="greater") %>%
...
```{r}
absoluteWilcoxResults <- momsptbDF %>%
  left_join(sgDF[,c("SampleID", "bacLoad")], by="SampleID") %>%
  filter(Cohort=="MOMS-PI", bacLoad<2) %>%
  mutate(absoluteAbundance=relativeAbundance*bacLoad) %>%
  with_groups(c(SubjectID, Cohort, TermPre, var), summarize, meanAA=mean(absoluteAbundance)) %>%
  group_by(var) %>%
  wilcox_test(meanAA~TermPre, alternative="greater") %>%
  adjust_pvalue(method = "BH")
...
momsptbDF %>%
  ...
  filter(Cohort=="MOMS-PI", bacLoad<2) %>%
  mutate(absoluteAbundance=relativeAbundance*bacLoad) %>%
  with_groups(c(SubjectID, Cohort, TermPre, var), summarize, Relative=mean(relativeAbundance), Absolute=mean(absoluteAbundance)) %>%
  gather("measure", "abundance", c(Relative, Absolute)) %>%
  mutate(var=factor(var, levels = c(clades, "Uncharacterized_Gardnerella",  "Gardnerella_vaginalis"), labels = c(clades, "Unch.", "Total"))) %>%
...
ggsave(file.path(figureOut, paste(Sys.Date(), "cladeAbundancePTB.png", sep="_")))
```
...
momsptbDF %>%
  ...
  theme(axis.text = element_text(size=13),
        strip.text = element_text(size=13),
        axis.title = element_text(size=16)) +
  labs(x=NULL, y="Mean Gestational Clade Count") +
  stat_pvalue_manual(nCladesWilcox, label="p", y.position = 6.7)
#ggsave(file.path(figureOut, paste(Sys.Date(), "cladeCountPTB.png", sep="_")))
```
...
momsptbDF %>%
  ...
  theme(axis.text = element_text(size=13),
        strip.text = element_text(size=13),
        axis.title = element_text(size=16)) +
  labs(x=NULL, y="Mean Gestational Bacterial Load") +
  stat_pvalue_manual(bacloadWilcox, label="p", y.position = 0.7)
#ggsave(file.path(figureOut, paste(Sys.Date(), "bacLoadByPTB.png", sep="_")))
```

## Enrichment for PTBs with enrichment for *Gardnerella*
```{r}
momsptbDF %>%
  select(SubjectID, Cohort, TermPre) %>%
  unique %>%
  ggplot(aes(x=Cohort, fill=TermPre)) +
  geom_bar(position = position_fill(), alpha=0.75, color="black") +
  binaryColors
#ggsave(file.path(figureOut, paste(Sys.Date(), "gardPTBenrichment.png", sep="_")))
```
...
```
  ...
  No difference between enriched vs. un-enriched births
