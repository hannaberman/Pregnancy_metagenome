...
## Define file paths
```{r, message=FALSE}
figureOut <- "../FIGURES/V2_manuscript_figures/cohorts_and_analyses"
...
```{r}
prevTable %>%
  mutate(var=factor(var, levels = c(clades, "Uncharacterized_Gardnerella",  "Gardnerella_vaginalis"))) %>%
...
tmp   %>%
  mutate(var=factor(var, levels=c("Gardnerella_vaginalis", "Uncharacterized_Gardnerella", rev(clades)), labels=c("Gv", "Unch.", rev(clades)))) %>%
  ggplot() +
  geom_pointrange(aes(y=var, x=estimate, xmin=conf.low, xmax=conf.high, group=Cohort, color=Cohort), position=position_dodge2(width=1, reverse=TRUE)) +
  theme_bw() +
...
gardCladeAbU %>%
  ...
  geom_text(data=prevTable, aes(x=1.25, label=prevalence), position=position_dodge2(width=1, reverse = TRUE), size=3) +
  theme_bw() +
  quadColors
...
genomosPrevTable <- gardGenomoAbU %>%
  ...
  gather("Genomospecies", "prevalence", c(genomos, "Uncharacterized_Gardnerella", "Gardnerella_vaginalis"))
...
gardGenomoAbU %>%
  ...
  group_by(Cohort, Genomospecies) %>%
  summarise(avgAbundance=mean(relativeAbundance)) %>%
  left_join(genomosPrevTable, by=c("Cohort", "Genomospecies")) %>%
...
sgDF %>%
  mutate(Cohort2=case_when(Cohort=="Stanford Enriched"|Cohort=="UAB Enriched"~"Stanford & UAB",
                           Cohort=="MOMS-PI"|Cohort=="MOMS-PI Enriched"~"MOMS-PI"),
         Cohort2=factor(Cohort2, levels=c("Stanford & UAB", "MOMS-PI"))) %>%
...
```{r}
# colnames(jaccards$`Stanford Enriched`) <- abbrevs
# rownames(jaccards$`Stanford Enriched`) <- abbrevs
# 
# (1 - jaccards$`Stanford Enriched`) %>%
#   corrplot(type="lower", is.corr = FALSE, diag = FALSE, tl.col = "black")
```
