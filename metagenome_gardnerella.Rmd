...
sgDF0 <- suDF %>%
  ...
  mutate(Cohort=factor(Cohort, levels = c("Stanford", "UAB", "HMP2"), labels=c("Stanford Enriched", "UAB Enriched", "HMP2")), # Stanford and UAB samples were selected for enrichment in Gardnerella, label them has Enriched.
...
demTable <- gaadDF %>%
  ...
  add_header_above(c(" ", "Stanford Enriched" = 2, "UAB Enriched" = 2, "HMP2" = 3)) %>%
  pack_rows("Race", 2, 5,) %>%
...
mDF1 %>%
  ...
  mutate(Cohort=case_when(Cohort=="Stanford Enriched"|Cohort=="UAB Enriched"~"Stanford & UAB Enriched",
                          Cohort=="HMP2"~"HMP2"),
         Cohort=factor(Cohort, levels = c("Stanford & UAB Enriched", "HMP2"))) %>%
  ggplot(aes(x=Gardnerella_vaginalis, color=Cohort, fill=Cohort, group=Cohort)) +
  geom_density(alpha=0.5) +
  binaryColors +
  labs(x="Proportion Gardnerella")
...
subjectGardAbundance2Cohort %>%
  filter(Cohort=="Stanford & UAB Enriched"|(Cohort=="HMP2"&SubjectID %in% HMP2highGardSubjects)) %>%
  mutate(Cohort=factor(Cohort, levels=c("Stanford & UAB Enriched", "HMP2"), labels=c("Stanford & UAB Enriched", "HMP2 Enriched"))) %>%
  ggplot(aes(x=Gardnerella_vaginalis, color=Cohort, fill=Cohort, group=Cohort)) +
  geom_density(alpha=0.5) +
  binaryColors +
  labs(x="Subject Average Proportion Gardnerella")
...
sgDF <- sgDF1 %>%
  ...
  mutate(Cohort=factor(Cohort, levels = c("Stanford Enriched", "UAB Enriched", "HMP2 Enriched", "HMP2")))
...
gardCladeAbU %>%
  mutate(var=factor(var, levels = c(clades, "Uncharacterized_Gardnerella",  "Gardnerella_vaginalis"), labels=c(clades, "Unch.", "Total")),
         Cohort=factor(Cohort, levels=c("Stanford Enriched", "UAB Enriched", "HMP2 Enriched", "HMP2"), labels = c("Stanford E", "UAB E", "HMP2 E", "HMP2"))) %>%
...
avgTop20Abundance0 %>%
  ...
  select(Species, `Stanford Enriched`, stanfordRank, `UAB Enriched`, uabRank, `HMP2 Enriched`, hmp2eRank, HMP2, hmp2Rank) %>%
...
jaccards %>%
  ...
  mutate(Var1=factor(Var1, levels=c("Gardnerella_vaginalis", clades, lactos, anaerobes), labels = abbrevs), 
         ...
         cohort=factor(cohort, levels = c("Stanford Enriched", "UAB Enriched", "HMP2 Enriched", "HMP2"))) %>%
