...
sam <- df %>%
  ...
  data.frame() %>%
  select(c(SubjectID, SampleID, GWcoll, RNAlater, SampleID_a, GWcoll_16S))
...
metaphlA_toi_o_df <- metaphlan_a_toi0 %>%
                        psmelt() %>%
                        filter(Abundance > 0) %>%
                        select(OTU, Abundance, SampleID, SubjectID, GWcoll, GWcoll_16S, Genus, Species, SampleID_a, RNAlater) %>%
