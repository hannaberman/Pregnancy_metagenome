...
mgsMetadata <- attrDF %>%
  ...
  left_join(readCountDF, by="SampleID") %>%
    mutate(sampleTrimester=case_when(!is.na(GDcoll)~case_when(GDcoll<=91~"First", # add trimester information based on GDcoll if present or reported trimester if not
