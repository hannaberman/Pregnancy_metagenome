...
#shotgun metadata
sgDF <- file.path(dataDir, "/METADATA/sumgsDF.tsv") %>%
  read_tsv %>%
...
gardAllVarDF %>%
  ...
  scale_y_log10() +
  facet_wrap(~SubjectID, nrow=2) +
  labs(x="Gestation weeks at collection", y="N Gardnerella Reads", shape="Method", color="Variant") +
  guides(color=guide_legend(override.aes=list(shape=15, size=3), ncol=2)) +
  theme(legend.position = "bottom",
        legend.direction = "vertical",
        legend.box = "horizontal",
...
cladeVs16SPlot <- gardAllVarDF_withThreshold %>%
  ...
  stat_cor(method = "pearson", alternative = "two.sided", cor.coef.name = c("R"), p.digits = 3, label.y = 0.95, label.x = 0) +
  geom_point(aes(color=Var, shape=`≥50% G3`), alpha=0.6, size=2) +
  scale_color_manual(values=c("#b22222", "#214fc6", "#E69F00", "#009E73")) +
  theme(legend.position = c(0.375, 0.375),
...
gsVs16SPlot <- gardAllVarDF_withThreshold %>%
  ...
  mutate(Var=as.character(Var),
         Var=case_when(Var=="Genomospecies 1, G. vaginalis"~"G2: Clades 1/2",
                       ...
                       Var=="Genomospecies 13"~"G5: Clade 6",
                       str_detect(Var, "G")~Var),
         Var=case_when(Var=="G2: Clades 1/2"~"G2: Gardnerella spp. 1/2/3/4/11",
                       Var=="G1: Clades 3/4"~"G1: Gardnerella spp. 5/6/7/8/9/10/14",
                       Var=="G4: Clade 5"~"G4: Gardnerella sp. 12",
                       Var=="G5: Clade 6"~"G5: Gardnerella sp. 13")) %>%
  group_by(SampleID, Method, Var) %>%
  summarise(propGard=sum(propGard)) %>%
  ungroup %>%
  spread(Method, propGard) %>%
  ...
  theme(legend.position = c(0.375, 0.375),
        ...
        legend.background = element_blank()) +
  guides(shape="none") +
  labs(color="V4 Variant", x="V4 region 16S amplicon", y="Shotgun Species")
gsVs16SPlot
```
...
cladeVsGsPlot <- gardAllVarDF_withThreshold %>%
  ...
  mutate(Var=as.character(Var),
         ...
         Var=case_when(Var=="Clade 1"~"Clade 1: G. vaginalis, Gard. sp. 2",
                       Var=="Clade 2"~"Clade 2: G. piottii, Gard. spp. 3/11",
                       Var=="Clade 3"~"Clade 3: Gard. spp. 7/8/9/10/14",
                       Var=="Clade 4"~"Clade 4: G. leopoldii, G. swidsinskii",
                       Var=="Clade 5"~"Clade 5: Gard. sp. 12",
                       Var=="Clade 6"~"Clade 6: Gard. sp. 13")) %>%
  group_by(SampleID, Method, Var) %>%
  summarise(propGard=sum(propGard)) %>%
  ungroup %>%
  spread(Method, propGard) %>%
  ...
  theme(legend.position = c(0.375, 0.375),
        ...
        legend.justification = c(0, 1),
        legend.direction = "vertical",
        legend.text = element_text(size=5, lineheight=0.2),
        legend.title = element_text(size=6),
        legend.background = element_blank()) +
  labs(color="Clade", x="Shotgun Clade", y="Shotgun Species")
cladeVsGsPlot
```

## Figure 2ABC:
```{r, message=FALSE, warning=FALSE, fig.width=8, fig.height=2.67}
# , fig.width=4, fig.height=1.3
...
# Samples where Gardnerella was detected by 16S but not shotgun sequencing
  In subject 10608: No Gardnerella was detected by our shotgun sequencing method, but was detected by 16S amplicon sequencing. Shotgun sequencing can demonstrate less sensitivity than amplicon sequencing.
