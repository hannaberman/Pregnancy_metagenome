...
cladesSpecies <- c("Lactobacillus_crispatus", "Lactobacillus_gasseri", "Lactobacillus_iners", "Lactobacillus_jensenii", "Gardnerella_vaginalis_C1", "Gardnerella_vaginalis_C2", "Gardnerella_vaginalis_C3", "Gardnerella_vaginalis_C4", "Gardnerella_vaginalis_C5", "Gardnerella_vaginalis_C6", "Atopobium_vaginae", "Mycoplasma_hominis", "Prevotella_bivia", "Ureaplasma_parvum")
...
cooccurr_analyses <- function(cohort, samples, organisms){  
  ...
  x0 <- x$results %>%
    select(sp1_name, sp2_name, obs_cooccur, exp_cooccur, 
  ...
  x3$sp1_name <- factor(x3$sp1_name, levels=organisms) #order x axis
  ...
  #add sp1 names column back and other columns
   x6 <- x5 %>%
     ...
     left_join(x3[,c("sp1_name", "sp2_name")], by=c("sp1_name", "sp2_name"))  %>% #"p_lt", "p_gt"
  ...
  title <- ifelse(length(organisms)==length(cladesSpecies), "Co-occurrence of G. vaginalis clades and key species", "Co-occurrence of total G. vaginalis and key species")
    ...
    plot <- ggplot(x6, aes(x=sp1_name, y=sp2_name, fill=effects)) + #label=pvalue
      ...
      labs(title=title, subtitle = paste0(cohort, ": ", samples), x=NULL, y=NULL, fill="Effect size")
