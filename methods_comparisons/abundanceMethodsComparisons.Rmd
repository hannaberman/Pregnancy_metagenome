...
#make list of taxa ratios (lactobacillus at species level, only)
#16S
...
meta_brack_16S_other0 <- meta_brack_16S_other %>%
  filter(Abundance_16S>max(S16values, na.rm = TRUE)) #filter on the 0.0395 value

#LOG RATIOS WITH 0.0395 CUTOFF
#16S
ratios16S0 <- matrix(nrow = 0, ncol = 3)
colnames(ratios16S0) <- c("SampleID_a", "Taxa", "logRatio_16S")
for(i in c(1:length(df$SampleID_a))){
  sam <- df$SampleID_a[i]
  for (j in c(1:dim(pairLG)[2])){
    a <- meta_brack_16S_other0$Abundance_16S[meta_brack_16S_other0$SampleID_a==sam & meta_brack_16S_other0$Taxon==pairLG[1,j]]
    b <- meta_brack_16S_other0$Abundance_16S[meta_brack_16S_other0$SampleID_a==sam & meta_brack_16S_other0$Taxon==pairLG[2,j]] 
    c <- a/b
    ifelse((length(c)>0), 
           d <- log10(c),
           d <- NA)
    e <- c(sam, paste0(pairLG[1,j], ":", pairLG[2,j]), d)
    ratios16S0 <- rbind(ratios16S0, e)
    }
}
ratios16S0[ratios16S0=="Inf"|ratios16S0=="-Inf"|ratios16S0=="NaN"] <- NA

#metaphlan
ratiosMetaphlan0 <- matrix(nrow = 0, ncol = 3)
colnames(ratiosMetaphlan0) <- c("SampleID_a", "Taxa", "logRatio_metaphlan")
for(i in c(1:length(df$SampleID_a))){
  sam <- df$SampleID_a[i]
  for (j in c(1:dim(pairLG)[2])){
    a <- meta_brack_16S_other0$Abundance_metaphl[meta_brack_16S_other0$SampleID_a==sam & meta_brack_16S_other0$Taxon==pairLG[1,j]]
    b <- meta_brack_16S_other0$Abundance_metaphl[meta_brack_16S_other0$SampleID_a==sam & meta_brack_16S_other0$Taxon==pairLG[2,j]] 
    c <- a/b
    ifelse((length(c)>0), 
           d <- log10(c),
           d <- NA)
    e <- c(sam, paste0(pairLG[1,j], ":", pairLG[2,j]), d)
    ratiosMetaphlan0 <- rbind(ratiosMetaphlan0, e)
    }
}
ratiosMetaphlan0[ratiosMetaphlan0=="Inf"|ratiosMetaphlan0=="-Inf"|ratiosMetaphlan0=="NaN"] <- NA
#merge tables
ratiosLG0 <- as.data.frame(ratios16S0) %>%
  full_join(as.data.frame(ratiosMetaphlan0), by = c("SampleID_a", "Taxa")) %>%
  mutate(logRatio_16S=as.numeric(as.character(logRatio_16S)),
         logRatio_metaphlan=as.numeric(as.character(logRatio_metaphlan)),
         RNAlater = case_when(SampleID_a %in% df$SampleID_a[df$RNAlater=="Yes"] ~ "Yes",
                              SampleID_a %in% df$SampleID_a[df$RNAlater=="No"] ~ "No"),
...
```{r}
meta_brack_16S_other1 <- meta_brack_16S_other %>%
  filter(Abundance_16S>0.02) #filter on 0.02

#LOG RATIOS WITH 0.02 CUTOFF
#16S
ratios16S1 <- matrix(nrow = 0, ncol = 3)
colnames(ratios16S1) <- c("SampleID_a", "Taxa", "logRatio_16S")
for(i in c(1:length(df$SampleID_a))){
  sam <- df$SampleID_a[i]
  for (j in c(1:dim(pairLG)[2])){
    a <- meta_brack_16S_other1$Abundance_16S[meta_brack_16S_other1$SampleID_a==sam & meta_brack_16S_other1$Taxon==pairLG[1,j]]
    b <- meta_brack_16S_other1$Abundance_16S[meta_brack_16S_other1$SampleID_a==sam & meta_brack_16S_other1$Taxon==pairLG[2,j]] 
    c <- a/b
    ifelse((length(c)>0), 
           d <- log10(c),
           d <- NA)
    e <- c(sam, paste0(pairLG[1,j], ":", pairLG[2,j]), d)
    ratios16S1 <- rbind(ratios16S1, e)
    }
}
ratios16S1[ratios16S1=="Inf"|ratios16S1=="-Inf"|ratios16S1=="NaN"] <- NA

#metaphlan
ratiosMetaphlan1 <- matrix(nrow = 0, ncol = 3)
colnames(ratiosMetaphlan1) <- c("SampleID_a", "Taxa", "logRatio_metaphlan")
for(i in c(1:length(df$SampleID_a))){
  sam <- df$SampleID_a[i]
  for (j in c(1:dim(pairLG)[2])){
    a <- meta_brack_16S_other1$Abundance_metaphl[meta_brack_16S_other1$SampleID_a==sam & meta_brack_16S_other1$Taxon==pairLG[1,j]]
    b <- meta_brack_16S_other1$Abundance_metaphl[meta_brack_16S_other1$SampleID_a==sam & meta_brack_16S_other1$Taxon==pairLG[2,j]] 
    c <- a/b
    ifelse((length(c)>0), 
           d <- log10(c),
           d <- NA)
    e <- c(sam, paste0(pairLG[1,j], ":", pairLG[2,j]), d)
    ratiosMetaphlan1 <- rbind(ratiosMetaphlan1, e)
    }
}
ratiosMetaphlan1[ratiosMetaphlan1=="Inf"|ratiosMetaphlan1=="-Inf"|ratiosMetaphlan1=="NaN"] <- NA
#merge tables
ratiosLG1 <- as.data.frame(ratios16S1) %>%
  full_join(as.data.frame(ratiosMetaphlan1), by = c("SampleID_a", "Taxa")) %>%
  mutate(logRatio_16S=as.numeric(as.character(logRatio_16S)),
         logRatio_metaphlan=as.numeric(as.character(logRatio_metaphlan)),
         RNAlater = case_when(SampleID_a %in% df$SampleID_a[df$RNAlater=="Yes"] ~ "Yes",
                              SampleID_a %in% df$SampleID_a[df$RNAlater=="No"] ~ "No"),
